...
def main():
    ...
    nor = [i['report'] for i in ann if i["label"]==0]
    ab = [i['report'] for i in ann if i["label"]!=0]

    top = int(len(tokenizer.token2idx_)*0.125)
    vob = [i for i in tokenizer.token2idx_]
    top_list = vob[:top]

    normal_dis = [0,0] #top,low
    for rr in nor:
        rr = tokenizer.clean_report(rr).split()
        for ww in rr:
            if ww in top_list:
                normal_dis[0] +=1
            else:
    ...
    print(np.array(normal_dis)/sum(normal_dis))
    print(np.array(abnormal_dis)/sum(abnormal_dis))
