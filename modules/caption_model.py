...
class CaptionModel(nn.Module):
    ...
    def beam_search(self, init_state, init_logprobs, *args, **kwargs):
        ...
        def beam_step(logprobs, unaug_logprobs, beam_size, t, beam_seq, beam_seq_logprobs, beam_logprobs_sum, state):
            ...
            beam_ix = torch.div(ix, vocab_size, rounding_mode='trunc') #ix // vocab_size  # Nxb which beam
        ...
        opt = kwargs['opt']
        distmem = kwargs['distmem']
        token_dist = kwargs['token_dist']
            ...
            for divm in range(group_size):
                if t >= divm and t <= self.max_seq_length + divm - 1:
                    ...
                    if token_dist is not None:
                        out = F.normalize(logprobs,dim=-1)
                        if t == 1:
                            token_dist = token_dist.repeat(len(logprobs)//batch_size,1)
                        logprobs = F.log_softmax(out*token_dist, dim=-1)
                        index = torch.argmax(logprobs,dim=-1).unsqueeze(-1)
                        prediction_mask = torch.zeros_like(out).cuda(0).scatter_(1, index, 1.)
                        next_prediction = prediction_mask * out
                        token_dist = token_dist - 1/60 * next_prediction
                    '''
                    ...
                    logprobs_table[divm], state_table[divm],_ = self.get_logprobs_state(distmem,it.cuda(self.device), *(
                            args[divm] + [state_table[divm]]),token_dist)
    ...
    def old_beam_search(self, init_state, init_logprobs,distmem=None, *args, **kwargs):
        ...
        def beam_step(logprobsf, unaug_logprobsf, beam_size, t, beam_seq, beam_seq_logprobs, beam_logprobs_sum, state,distmem=None):
            # INPUTS:
        ...
        for t in range(self.max_seq_length + group_size - 1):
            for divm in range(group_size):
                if t >= divm and t <= self.max_seq_length + divm - 1:
                    ...
                    # suppress previous word
                    if decoding_constraint and t - divm > 0:
                        logprobsf.scatter_(1, beam_seq_table[divm][t - divm - 1].unsqueeze(1).cuda(self.device), float('-inf'))
                    # suppress UNK tokens in the decoding
                    ...
                    logprobs_table[divm], state_table[divm] = self.get_logprobs_state(distmem,it.cuda(self.device), *(
    ...
    def sample_next_word(self, logprobs, sample_method, temperature):
        if sample_method == 'greedy':
            ...
            def sample_gumbel(shape, eps=1e-20):
                U = torch.rand(shape).cuda(self.device)
