...
class DistributionMemory(nn.Module):

    def __init__(self, vob_size):
        super(DistributionMemory, self).__init__()

        self.w_v = nn.Linear(vob_size,vob_size)
        self.att = torch.nn.MultiheadAttention(vob_size,1,batch_first=False)

        self.w_i = nn.Linear(vob_size,vob_size)
        self.W = nn.Linear(vob_size, vob_size)
        self.U = nn.Linear(vob_size, vob_size)
        self.vob_size = vob_size


    def forward_step(self, p_t_1, td_t_1):

        p_t_1 = F.one_hot(p_t_1,num_classes=self.vob_size).float()
        p_t, _ = self.att(p_t_1.unsqueeze(1),p_t_1.unsqueeze(1),td_t_1.unsqueeze(1))
        p_t = p_t.squeeze(1)

        input_gate = self.W(p_t_1-td_t_1)
        forget_gate = self.U(torch.tanh(p_t_1-td_t_1))
        input_gate = torch.sigmoid(input_gate)
        forget_gate = torch.sigmoid(forget_gate)

        next_memory = input_gate * torch.tanh(p_t_1) - forget_gate * p_t_1
        return F.log_softmax(p_t,dim=-1), torch.sigmoid(next_memory)

    def forward(self, inputs, memory):
        outputs = []
        p_t = torch.zeros_like(memory).long().cuda(1)
        #p_t = F.log_softmax(memory,dim=-1)
        outputs.append(p_t)
        for i in range(1,inputs.shape[1]):
            p_t, memory = self.forward_step(inputs[:, i], memory)
            outputs.append(p_t)
...
class EncoderDecoder(AttModel):
    ...
    def __init__(self, args, tokenizer):
        ...
        #self.distribution_memory  = DistributionMemory(tgt_vocab)
        #self.token_dis = nn.Linear(4096, tgt_vocab)
    ...
    def _forward(self, fc_feats, att_feats, seq, att_masks=None,token_dist=None):

        #token_dist =  torch.sigmoid(self.token_dis(fc_feats))
        att_feats, seq, att_masks, seq_mask = self._prepare_feature_forward(att_feats, att_masks, seq)
        out = self.model(att_feats, seq, att_masks, seq_mask)
        outputs = F.log_softmax(self.logit(out),dim=-1)
        #outputs_adj = self.distribution_memory(seq,token_dist)
        #outputs_adj = outputs_adj + outputs
