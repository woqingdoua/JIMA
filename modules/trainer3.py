...
from models.r2gen import DistMemModel
from modules.loss import compute_loss2
...
class BaseTrainer(object):
    def __init__(self, model, criterion, metric_ftns, optimizer, args):
        ...
        pretrained_params = torch.load('/hdd18t/yuexin/Documents/r2genbaseline/classification/best_classifier_mimic.pkl')
    ...
    def _save_checkpoint(self, epoch, save_best=False):
        ...
        filename = os.path.join(self.checkpoint_dir, 'mimic_my_checkpoint.pth')
        torch.save(state, filename)
        print("Saving checkpoint: {} ...".format(filename))
        if save_best:
            best_path = os.path.join(self.checkpoint_dir, 'mimic_my_model_best.pth')
            torch.save(state, best_path)
            print("Saving current best: mimic_my_model_best.pth ...")
    ...
    def _resume_checkpoint(self,resume_path='/home/ywu10/Documents/r2genbaseline/'):# resume_path='/hdd18t/models/model_iu_xray.pth'):
        ...
        resume_path = '/home/ywu10/Documents/r2genbaseline/mimic_cxr_wcl.pth'
...
class Trainer(BaseTrainer):
    def __init__(self, model, criterion, metric_ftns, optimizer, args, lr_scheduler, train_dataloader, val_dataloader,
        ...
        self.dist_mem = DistMemModel(model.tokenizer).to(self.device)
        self.optimizer2 = torch.optim.Adam(self.dist_mem.parameters(), lr=0.0004)

    def pretrain_epoch1(self, epoch):

        train_loss = 0
        self.model.train()
        for batch_idx, (images_id, images, reports_ids, reports_masks,_,_, reports_labels) in enumerate(self.train_dataloader):
            images, reports_ids, reports_masks = images.to(self.device), reports_ids.to(self.device), reports_masks.to(
                self.device)
            reports_labels = reports_labels.to(self.device)
            output,_ = self.model(images, reports_ids, mode='train')
            loss = compute_loss2(self.model, output, reports_ids, reports_masks, reports_labels, epoch, self.args)
    ...
    def _train_epoch2(self, epoch):
        ...
        for batch_idx, (images_id, images, reports_ids, reports_masks,_,_, label) in enumerate(self.train_dataloader):
            ...
            if batch_idx%10000 == 0:
                    log = self._train_epoch2(epoch)

            '''
            output,fc_feats = self.model(images, reports_ids, mode='train')
            output, token_dist = self.dist_mem(output[0],fc_feats)
            reports_ids_one_hot = F.one_hot(reports_ids,output.size()[-1])
            reports_ids_one_hot = torch.sum(reports_ids_one_hot,dim=1)
            loss = F.binary_cross_entropy_with_logits(token_dist,(reports_ids_one_hot>0).float(),weight=reports_ids_one_hot)
            ...
            output,fc_feats = self.model(images, reports_ids, mode='train')
            output, token_dist = self.dist_mem(output[0],fc_feats)
            loss = self.criterion(F.log_softmax(output,dim=-1), reports_ids, reports_masks,weight=None)
            train_loss += loss.item()
            self.optimizer2.zero_grad()
            loss.backward()
            #torch.nn.utils.clip_grad_value_(self.dist_mem.parameters(), 0.1)
            self.optimizer2.step()
            j += 1
            p.update(j)
        p.finish()
    ...
    def evaluate(self):
        ...
        self.model.eval()
        p = progressbar.ProgressBar(len(self.train_dataloader))
        p.start()
        j = 0
        with torch.no_grad():
            for batch_idx, (images_id, images, reports_ids, reports_masks, bert_reports_ids, bert_reports_masks,label) in p(enumerate(self.train_dataloader)):
                ...
                output,fc_feats = self.model(images,reports_ids,  mode='train')
                output =  output[0]
                output1 = torch.argmax(self.dist_mem(output,fc_feats)[0],dim=-1)
                output1 = self.model.tokenizer.decode_batch(output1.cpu().numpy())
                output2 = torch.argmax(output,dim=-1)
                output2 = self.model.tokenizer.decode_batch(output2.cpu().numpy())
                ...
                for i in range(len(reports)):
                    b4 = self.compute_b4({0: [ground_truths[i]]},{0: [output1[i]]})
                    ...
                    self.train_dataloader2.dataset.examples[index]['BLEU4'] = b4['BLEU_4']
    ...
    def _train_epoch(self, epoch):
        ...
        ad_ratio = 0
        total = 0
        #log = self.pretrain_epoch1(epoch)
        ...
        for batch_idx, (task1_image_id,task1_image, task1_targets, task1_targets_masks, \
                        ...
                        task2_bert_targets, task2_bert_targets_masks, task2_label, \
                        task3_image_id,task3_image, task3_targets, task3_targets_masks) in p(enumerate(self.train_dataloader2)):
            ...
            output,fc_feats = self.model(task1_image,  task1_targets, mode='train')
            output, token_dist = self.dist_mem(output[0],fc_feats)
            ...
            output,_ = self.model(task2_image, task2_targets, mode='train')
            ...
            loss = self.criterion(output, task2_targets, task2_targets_masks,weight=weight)
            train_loss += loss.item()
            self.optimizer.zero_grad()
            loss.backward()
            torch.nn.utils.clip_grad_value_(self.model.parameters(), 0.1)
            ...
            #task3
            task3_image, task3_targets, task3_targets_masks = task3_image.to(self.device), task3_targets.to(self.device), task3_targets_masks.to(self.device)

            output,token_dist = self.model(task3_image, task3_targets, mode='train')
            output = self.dist_mem(output[0],token_dist)[0]
            loss = self.criterion(F.log_softmax(output,dim=-1), task3_targets, task3_targets_masks,weight=None)
            train_loss += loss.item()
            self.optimizer2.zero_grad()
            loss.backward()
            #torch.nn.utils.clip_grad_value_(self.model.parameters(), 0.1)
        ...
        with torch.no_grad():
            ...
            for batch_idx, (images_id, images, reports_ids, reports_masks,_,_, label) in enumerate(self.test_dataloader):
                ...
                output,_,_ = self.model(images,distmem=self.dist_mem, mode='sample')
                reports = self.model.tokenizer.decode_batch(output.cpu().numpy())
                ground_truths = self.model.tokenizer.decode_batch(reports_ids[:, 1:].cpu().numpy())
        ...
        with torch.no_grad():
            ...
            for batch_idx, (images_id, images, reports_ids, reports_masks,_,_, label) in enumerate(self.test_dataloader):
                images, reports_ids, reports_masks = images.to(self.device), reports_ids.to(
                    self.device), reports_masks.to(self.device)
                output,prob,fc_feats = self.model(images, distmem=self.dist_mem,mode='sample')
                output,_ = self.dist_mem(prob,fc_feats)
                output = torch.argmax(output,dim=-1)
                reports = self.model.tokenizer.decode_batch(output.cpu().numpy())
                ground_truths = self.model.tokenizer.decode_batch(reports_ids[:, 1:].cpu().numpy())
