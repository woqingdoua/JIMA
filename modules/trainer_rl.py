...
class BaseTrainer(object):
    ...
    def _save_checkpoint(self, epoch, save_best=False):
        ...
        filename = os.path.join(self.checkpoint_dir, 'rl_current_checkpoint.pth')
        torch.save(state, filename)
        self.logger.info("Saving checkpoint: {} ...".format(filename))
        if save_best:
            best_path = os.path.join(self.checkpoint_dir, 'rl_model_best.pth')
            torch.save(state, best_path)
            self.logger.info("Saving current best: rl_model_best.pth ...")
...
class Trainer(BaseTrainer):
    ...
    def _train_epoch(self, epoch):
        ...
        train_loss = 0
        self.model.train()
        for batch_idx, (images_id, images, reports_ids, reports_masks,_,_,_) in enumerate(self.train_dataloader):
            ...
            if (batch_idx+1) % self.args.sc_eval_period == 0:
                ...
                with torch.no_grad():
                    # val_loss = 0
                    val_gts, val_res = [], []
                    for batch_idx, (images_id, images, reports_ids, reports_masks,_,_,_) in enumerate(self.val_dataloader):
                ...
                with torch.no_grad():
                    test_gts, test_res = [], []
                    for batch_idx, (images_id, images, reports_ids, reports_masks,_,_,_) in enumerate(self.test_dataloader):
                        images, reports_ids, reports_masks = images.to(self.device), reports_ids.to(
                            self.device), reports_masks.to(self.device)
                        output, _ = self.model(images, mode='sample')
                        reports = self.model.tokenizer.decode_batch(output.cpu().numpy())
        ...
        file_name = "/home/ywu10/Documents/r2genbaseline/results/mimic_cxr_rl_pre_report.csv"
        with open(file_name,"w") as csvfile:
            writer = csv.writer(csvfile)
            writer.writerows(test_res1)

        label_list = label_list.cpu().numpy()
        file = open('/home/ywu10/Documents/r2genbaseline/results/mimic_cxr_rl_report.pkl', 'wb')
